...
void launch_patched(const torch::Tensor& logits, const torch::Tensor& lengths,
  ...
  // Match the production dispatcher for every compiled implementation.
  // Keeping this decision in the common wrapper makes MAIN, the existing
  // exact fix, and the candidate differ only in their included headers.
  if (num_rows > 32 && properties.sharedMemPerBlockOptin >= 128 * 1024) {
    check_cuda(vllm::FilteredTopKRaggedTransform<float, int32_t, TopK>(
               ...
               "FilteredTopK failed");
    check_cuda(cudaGetLastError(), "production-dispatch top-k launch failed");
    return;
  }
